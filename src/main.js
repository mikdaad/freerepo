...
function handleAction(action) {
  ...
  if (action === 'handoff') {
    const job = state.jobs.find((item) => item.id === state.selectedJob);
    if (!job) return;
    window.postMessage({ source: 'orbit-app', type: 'orbit:handoff', prompt: buildArenaPrompt(job) }, '*');
    state.toast = 'Sending the tailored brief to your Arena.ai tab…';
    render();
    setTimeout(() => { if (state.toast === 'Sending the tailored brief to your Arena.ai tab…') { state.toast = 'Brief prepared — install the connector to hand it off'; render(); } }, 1800);
    setTimeout(() => { state.toast = null; render(); }, 4200);
    return;
  }
  if (action === 'analyze') {
...
window.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'orbit:job-captured' && event.data.job) {
    ...
    state.toast = 'Arena.ai is focused with your brief ready';
    render();
    setTimeout(() => { state.toast = null; render(); }, 3200);
  }
});
